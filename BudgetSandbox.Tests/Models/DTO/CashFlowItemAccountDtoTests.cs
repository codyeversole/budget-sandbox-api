...
{
    public class CashFlowItemAccountDtoTests
    {
        public static IEnumerable<object[]> ValidModels => new List<object[]>
        {
            new object[] { new CashFlowItemAccountDto { AccountId = 1, Amount = 50m } },
            new object[] { new CashFlowItemAccountDto { AccountId = 1, Percent = 1 } }
        };

        public static IEnumerable<object[]> InvalidModels => new List<object[]>
        {
            new object[] { new CashFlowItemAccountDto { AccountId = 1 } },
            new object[] { new CashFlowItemAccountDto { AccountId = 1, Amount = 50m, Percent = 1 } }
        };

        [Theory]
        [MemberData(nameof(ValidModels))]
        public void Should_be_valid(CashFlowItemAccountDto model)
        {
            //Arrange
        ...
        public void Should_be_invalid(CashFlowItemAccountDto model)
        {
            //Arrange
            var validationResultList = new List<ValidationResult>();
